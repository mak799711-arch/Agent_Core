...
export default function MapPicker({ initialLat, initialLng, initialAddress, onLocationSelect, theme = 'dark' }: MapPickerProps) {
  ...
  useEffect(() => {
    ...
    function initMap() {
      ...
      const defaultLat = initialLat || -8.6500;
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', height: '100%' }}>
      ...
      {results.length > 0 && (
        <div style={{ background: 'var(--glass-bg)', border: '1px solid var(--surface-border)', borderRadius: '8px', maxHeight: '200px', overflowY: 'auto' }}>
          {results.map((r, i) => (
            <div 
              key={i} 
      ...
      <div ref={mapContainer} style={{ flex: 1, minHeight: '400px', borderRadius: '12px', border: '1px solid var(--surface-border)', zIndex: 0 }}></div>
      <p style={{ fontSize: '0.8rem', opacity: 0.6 }}>* You can also click directly on the map to pin your exact location manually.</p>
    </div>
  );
}
